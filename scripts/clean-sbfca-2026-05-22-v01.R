...
paths <- submission_paths("sbfca", "2026-05-22-v01")
source_layer <- "SBFCA_OWA_Robinsons_Riffle"
...
cleaned <- raw |>
  dplyr::mutate(
    ...
    contractors = dplyr::case_when(
      normalize_semicolon_values(contractors) == "R&F; Cramer" ~ "R&F Engineering; Cramer Fish Sciences",
      TRUE ~ normalize_semicolon_values(contractors)
    ),
    ...
    funding_sources = "Wildlife Conservation Board Greenhouse Gas Reduction Fund",
...
serialized_changes <- raw |>
  ...
  dplyr::transmute(
    ...
    project_stage_submitted = project_stage,
    project_stage_standardized = cleaned$project_stage,
    project_type_submitted = project_type,
    project_type_standardized = cleaned$project_type,
    contractors_submitted = contractors,
    contractors_standardized = cleaned$contractors,
    target_species_submitted = target_species,
    target_species_standardized = cleaned$target_species,
    funding_sources_submitted = funding_sources,
    funding_sources_standardized = cleaned$funding_sources
  )

for (field in c("project_stage", "project_type", "contractors", "target_species", "funding_sources")) {
  ...
  for (i in seq_len(nrow(field_changes))) {
    validation <- append_validation(
      ...
      dplyr::case_when(
        field == "contractors" ~ "Standardized confirmed contractor organization names.",
        field == "funding_sources" ~ "Standardized to confirmed funding source name.",
        TRUE ~ "Normalized semicolon-delimited serialization while preserving submitted values."
      )
    )
  }
}
...
write_qc_report(
  path = paths$qc_md,
  title = "SBFCA 2026-05-22-v01 QC Report",
  ...
  omitted_source_fields = c("Shape_Leng", "Shape")
)
