...
paths <- submission_paths("sfpuc", "2026-05-22-v01")
source_layer <- "merged"
...
project_name_map <- c(
  "Upstream of Old LaGrange Bridge Restoration Projec" =
    "Upstream of Old LaGrange Bridge Restoration Project"
)

lead_entity_map <- c(
  "San Francisco Public Utilites Commission; Modesto Irrigation District; Turlock Irrigation District" =
    "San Francisco Public Utilities Commission; Modesto Irrigation District; Turlock Irrigation District"
)

funding_sources_map <- c(
...
cleaned <- raw |>
  dplyr::mutate(
    ...
    lead_entity = dplyr::recode(lead_entity, !!!lead_entity_map, .default = lead_entity),
    ...
    funding_sources = dplyr::recode(funding_sources, !!!funding_sources_map, .default = funding_sources),
...
validation <- append_validation(
  ...
  "info",
  "source_inventory",
  message = "Submitted GeoPackage contains a second layer with matching feature count and fields; `merged` was used as the source layer."
)
...
project_name_changes <- raw |>
  sf::st_drop_geometry() |>
  dplyr::transmute(
    feature_id = project_name,
    submitted = project_name,
    standardized = cleaned$project_name
  ) |>
  dplyr::filter(submitted != standardized)

for (i in seq_len(nrow(project_name_changes))) {
  validation <- append_validation(
    validation, "warning", "value_repair", project_name_changes$feature_id[[i]], "project_name",
    project_name_changes$submitted[[i]], project_name_changes$standardized[[i]],
    "Repaired apparent field-width truncation in submitted project name."
  )
}

lead_entity_changes <- raw |>
  sf::st_drop_geometry() |>
  dplyr::transmute(
    feature_id = cleaned$project_name,
    submitted = lead_entity,
    standardized = cleaned$lead_entity
  ) |>
  dplyr::filter(submitted != standardized)

for (i in seq_len(nrow(lead_entity_changes))) {
  validation <- append_validation(
    validation, "warning", "value_repair", lead_entity_changes$feature_id[[i]], "lead_entity",
    lead_entity_changes$submitted[[i]], lead_entity_changes$standardized[[i]],
    "Corrected apparent spelling error in lead-entity organization name and normalized semicolon serialization."
  )
}
...
for (i in seq_len(nrow(funding_source_changes))) {
  validation <- append_validation(
    ...
    "Converted comma-delimited funder acronyms to semicolon-delimited values and corrected apparent SFPUC typo."
  )
}

serialized_changes <- raw |>
  sf::st_drop_geometry() |>
  dplyr::transmute(
    feature_id = cleaned$project_name,
    project_type_submitted = project_type,
    project_type_standardized = cleaned$project_type,
    target_species_submitted = target_species,
    target_species_standardized = cleaned$target_species
  )

for (field in c("project_type", "target_species")) {
  submitted_col <- paste0(field, "_submitted")
  standardized_col <- paste0(field, "_standardized")
  field_changes <- serialized_changes |>
    dplyr::filter(.data[[submitted_col]] != .data[[standardized_col]])

  for (i in seq_len(nrow(field_changes))) {
    validation <- append_validation(
      validation, "warning", "value_repair", field_changes$feature_id[[i]], field,
      field_changes[[submitted_col]][[i]], field_changes[[standardized_col]][[i]],
      "Normalized semicolon-delimited serialization while preserving submitted values."
    )
  }
}
...
fish_food_without_acres <- cleaned |>
  sf::st_drop_geometry() |>
  dplyr::filter(
    stringr::str_detect(project_type, "(^|; )fish food production($|;)"),
    is.na(acreage_fish_food)
  )

for (i in seq_len(nrow(fish_food_without_acres))) {
  validation <- append_validation(
    validation, "info", "acreage_consistency", fish_food_without_acres$project_name[[i]],
    "acreage_fish_food", NA_character_, NA_character_,
    "Project type includes fish food production, but submitted acreage_fish_food is missing; retained as missing by local review decision."
  )
}
...
write_qc_report(
  path = paths$qc_md,
  title = "SFPUC 2026-05-22-v01 QC Report",
  ...
  omitted_source_fields = "geometry"
)
