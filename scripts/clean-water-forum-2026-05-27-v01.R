...
paths <- submission_paths("water-forum", "2026-05-27-v01")
source_layer <- "LAR_HRL_Project_Info"
...
submitted_na_to_missing <- function(x) {
  x <- null_to_na_chr(x)
  dplyr::if_else(stringr::str_to_lower(x) %in% c("n/a", "na"), NA_character_, x)
}

cleaned <- raw |>
  dplyr::mutate(
    submitted_early_implementation = early_implementation,
    submitted_estimated_budget_comments = estimated_budget_comments,
    ...
    early_implementation = dplyr::case_when(
      stringr::str_to_lower(null_to_na_chr(early_implementation)) == "yes" ~ TRUE,
      stringr::str_to_lower(null_to_na_chr(early_implementation)) == "no" ~ FALSE,
      TRUE ~ NA
    ),
    ...
    construction_completion_year_comments =
      stringr::str_trunc(null_to_na_chr(construction_completion_year_co), 250, ellipsis = ""),
    estimated_budget = as.integer(round(estimated_budget)),
    estimated_budget_comments = stringr::str_trunc(submitted_na_to_missing(estimated_budget_comments), 500, ellipsis = ""),
...
for (i in seq_len(nrow(early_implementation_changes))) {
  validation <- append_validation(
    ...
    "Converted submitted yes/no text to schema boolean value."
  )
}

estimated_budget_comment_changes <- raw |>
  sf::st_drop_geometry() |>
  dplyr::transmute(
    feature_id = project_name,
    submitted = estimated_budget_comments,
    standardized = cleaned$estimated_budget_comments
  ) |>
  dplyr::filter(!is.na(submitted_na_to_missing(submitted)) != !is.na(standardized) | submitted != standardized)

for (i in seq_len(nrow(estimated_budget_comment_changes))) {
  validation <- append_validation(
    validation, "warning", "value_repair", estimated_budget_comment_changes$feature_id[[i]],
    "estimated_budget_comments",
    estimated_budget_comment_changes$submitted[[i]], estimated_budget_comment_changes$standardized[[i]],
    "Treated submitted N/A budget comment as missing."
  )
}
...
write_qc_report(
  path = paths$qc_md,
  title = "Water Forum 2026-05-27-v01 QC Report",
