...
report_dir <- "reports/proposition-name-canonicalization"
...
canonicalize_values <- function(values) {
  purrr::map_chr(values, function(value) {
    ...
    gsub("(?<![[:alnum:]])Prop(?![[:alnum:]])", "Proposition", value, perl = TRUE)
  })
}
...
applied <- purrr::pmap_dfr(submissions, function(source_slug, source_file) {
  ...
  character_fields <- setdiff(names(attributes)[vapply(attributes, is.character, logical(1))], "project_name")

  audit <- attributes |>
    dplyr::mutate(source_slug = source_slug, .before = 1) |>
    dplyr::select(source_slug, project_name, dplyr::all_of(character_fields)) |>
    tidyr::pivot_longer(
      dplyr::all_of(character_fields),
      names_to = "field",
  ...
  data <- data |>
    dplyr::mutate(dplyr::across(dplyr::all_of(character_fields), canonicalize_values))
...
readr::write_lines(
  c(
    paste0("# Proposition Name Canonicalization ", report_date),
    "",
    "## Summary",
    "",
    paste0("- Replaced standalone `Prop` with `Proposition` in ", nrow(applied), " standardized text-field value(s)."),
...
message("Applied ", nrow(applied), " Proposition-name updates.")
